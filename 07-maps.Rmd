...
trees_raw <- read_csv("data/lab07/Parks_Tree_Inventory.csv")
glimpse(trees_raw)
```
...
trees_shape <- st_read("data/lab07/Parks_Tree_Inventory-shp")
```
...
pdx_boundaries <- st_read("data/lab07/Neighborhoods__Regions_-shp")
```
...
river_boundaries <- st_read("data/lab07/Willamette_Columbia_River_Ordinary_High_Water-shp/")
```
...
Note that, at this point, `p1` is just a regular dataframe, without any geospatial information. _We_ know that it contains coordinate points on the WGS84 ellipsoid, but nobody _else_ looking at those points (including any R functions) will, so we have some work to do before we can actually use this point for anything.

We begin by figuring out the EPSG numbers for our source and target CRS. 
Our source CRS is WGS84, whose CRS number is `4326`; let's say that we wanted to project it into the same space used by our dataset of Portland neighborhood boundaries, so that we could overlay the points or do some other sort of calculation. The Portland neighborhood maps dataset uses EPSG `3857`, also known as "Web Mercator". This is basically WGS84, but it has been "quasi-Mercator projected," and converted to use meters as units and easting/northing as its direction system (instead of lat/long). 

<div class="panel panel-info">
  <div class="panel-heading">Web Mercator? WTF?</div>
  <div class="panel-body">

For better or worse, Web Mercator is a very important CRS to become familiar with, as it is used by a variety of mapping software packages (including Google Maps, etc.), and is also the CRS used by the City of Portland's geospatial data portal. 
The motivation behind Web Mercator is that it depends on a _spherical_ projection geometry (rather than the _ellipsoidal_ projection used by "vanilla" Mercator). This makes it vastly simpler to work with from a computational perspective... at the cost of substantially decreased accuracy at northern latitudes, even by the questionable standards of the Mercator projection. 

For this reason, the CRS's metadata includes a `USAGE` note saying that it is only specified for use at latitudes between 85º N and 85º S. In practice, it is best to not push things even that far; you'll see notable distortion by the time you get to Norway and Alaska. 

  </div>
</div>


Now that we know which CRS systems are in play, we can use `sf_project()` to convert our "raw" WGS84 spherical coordinates into projected Web Mercator coordinates:
...
Besides specifying a CRS by its EPSG nubmer, another common way to define a CRS is with a "`PROJ.4`" string. Instead of referring to a standard, predefined projection as with an EPSG code, the proj.4 string includes all of the necessary information to describe an arbitrary projection. Consider this example, from the `geom_sf` documentation:
...
trees_shape <- st_read("data/lab07/Parks_Tree_Inventory-shp")
farmers_market <- st_read("data/lab07/Farmers_Markets-shp/")
grocery_stores <- st_read("data/lab07/Grocery_Stores-shp/")
...
Note that for exploring this part of the data, we may wish to bring in additional layers- perhaps the routes of I-5 and I-205. Other major arterial roads may also be important (Foster, Powell, and 82nd on the East side come to mind, as do Barbur and Capitol Highway on the West side). 

With this many data points, it may begin to make sense to look not at discrete points but rather at a density or contour plot. In ggplot, the easiest way to do this is with the `stat_density_2d` geom, but with geospatial dataframes there is a complication: this geom needs the coordinates for each datapoint to be separated into two columns (to map to the `x` and `y` aesthetics), and our dataframe is full of `sf` points.

<div class="panel panel-warning">
  <div class="panel-heading">What about the existing `X` and `Y` columns?</div>
  <div class="panel-body">
If you look closely at our `grocery_stores` dataframe, you might notice that in addition to the `geometry` column used by `geom_sf`, we also have `X` and `Y` columns.
Obviously, these are the coordinates for each of our grocery stores, but... in what CRS?
    
This is an example of a common problem with geospatial data: since we don't know the coordinate system that these columns are using, they are useless for our current purpose! 😣

In this case, with enough detective work, we can use the [dataset's metadata](https://www.portlandmaps.com/metadata/index.cfm?&action=DisplayLayer&LayerID=54182) to work out what those columns mean: they are coordinates in the [State Plane Coordinate System](https://en.wikipedia.org/wiki/State_Plane_Coordinate_System), measured in feet.

From here, if you _really_ needed to, you would be able to convert these points into the CRS used by the rest of our data...
  </div>
</div>

Rather than using the "mystery" `X` and `Y` columns, we can use the geospatially-tagged data in the `geometry` column, from which we can extract "normal" R numeric data using `st_coordinates`:

```{r}
grocery_stores$geometry %>% head %>% st_coordinates
```
Note that these are clearly Web Mercator-projected coordinates, just like we need for our map and for stat_density_2d!

From here, we can feed these points directly to `stat_density_2d`:
...
pdx_boundaries %>% ggplot() +
  ...
  stat_density_2d(
    ...
    mapping=aes(X,Y), # remember. this aesthetic mapping is "local" to this geom
    color="red", 
    alpha=0.5)  +
...
Now, we are ready to try plotting the trees over our base map. Our trees and basemap are in different projections, and `ggmap` can't quite handle that situation "on its own":

- The trees, coming from the city's geodata portal, are _projected_ Web Mercator coordinates 
- `ggmap` wants _unprojected_ WGS84 coordinates.

We can use `st_transform` to manually re-project our trees' locations:
...
pdx_population <- read_excel("data/lab07/Census_2010_Data_Cleanedup.xlsx",
